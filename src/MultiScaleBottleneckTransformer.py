...
class MultiScale_Bottleneck_Transformer(nn.Module):
    ...
    def forward(self, m_a, m_b):
        ...
        for layer_i in range(self.n_layers):
            m_a_cat = torch.cat([m_a_in, bottleneck], dim=1)    # 模型a与bottleneck拼接
            ...
            if layer_i < self.n_layers - 1:
                ...
                next_bottleneck = next_bottleneck.repeat([n_batch, 1, 1])   # 初始化下一层的bottleneck
                bottleneck = self.decoder(next_bottleneck, m_a_bottleneck, m_a_bottleneck)  # 交叉注意力处理下一层bottleneck
            m_b_cat = torch.cat([m_b_in, m_a_bottleneck], dim=1)    # 本层bottleneck和模态b拼接
            m_b_cat = self.sma[layer_i](m_b_cat, m_b_cat, m_b_cat)  # 进入TransformerLayer
