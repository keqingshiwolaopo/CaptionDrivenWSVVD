...
for idx in range(len(lists)):
    ...
    name = name[:-7]
    vname = name+'.mp4'
    cap = cv2.VideoCapture(os.path.join(video_root,vname))  #   打开对应的视频文件
    lens = int(cap.get(7)) # 获取视频的总帧数

    # the number of testing images in this sub-dir

    gt_vec = np.zeros(lens).astype(np.float32)  # 初始化标注向量，长度为视频帧数，初始值全为 0
    ...
    gt_vec = gt_vec[:-1]     # 去掉最后一个帧，保证与 flow rgb 特征对齐
    if mod:
        gt_vec = gt_vec[:-mod]    # 去掉多余的帧，确保帧数是 clip_len 的倍数
    gt.extend(gt_vec)
